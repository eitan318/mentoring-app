...
namespace MentoringApp.Service
{
    public class ReviewService
    {
        ...
        public async Task<Result> CreateReviewAsync(string content, int pairId, int authorUserId)
        {
            if (string.IsNullOrWhiteSpace(content))
                return Result.Failure("Review content cannot be empty.");

            bool created = await _reviewRepo.CreateAsync(content, DateTime.UtcNow, pairId, authorUserId);
