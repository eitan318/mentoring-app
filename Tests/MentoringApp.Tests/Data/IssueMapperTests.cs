...
namespace MentoringApp.Tests.Data
{
    ...
    {
        private static IssueDao MakeDto(int id=1,string description="Test issue",int categoryId=10,int reportedByUserId=7,int isResolved=0,string creationDate="2024-01-15T10:30:00") =>
            new IssueDao{Id=id,Description=description,CategoryId=categoryId,ReportedByUserId=reportedByUserId,IsResolved=isResolved,CreationDate=creationDate};
        ...
        [Fact] public void ToModels_ReturnsEmpty_ForEmptyInput(){IssueMapper.ToModels(Array.Empty<IssueDao>(),new[]{MakeCategory()}).Should().BeEmpty();}
    }
}
